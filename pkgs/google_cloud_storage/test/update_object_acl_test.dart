...
void updateObjectAclTest(Storage Function() createStorage) {
  ...
  test('success', () async {
    ...
    final acl = await storage.updateObjectAcl(
      bucketName,
      'object.txt',
      entity,
      'OWNER',
  ...
  test('no object', () async {
    final bucketName = await createBucketWithTearDown(
      storage,
      'upd_obj_acl_no_object',
    );

    expect(
      () => storage.updateObjectAcl(
        bucketName,
        'non-existent.txt',
        'allUsers',
        'READER',
...
void main() async {
  group('update object acl', () {
    group('google-cloud', tags: ['google-cloud', 'no-ulba'], () {
      updateObjectAclTest(Storage.new);
    });
    ...
    test('non-idempotent transport failure', () async {
      ...
      await expectLater(
        storage.updateObjectAcl('bucket', 'object', 'entity', 'role'),
