...
/// Generates a sequence of wait durations for exponential backoff.
///
/// The sequence respects [RetrySettings.maxRetries] and [RetrySettings.maxRetryInterval].
/// Each delay is calculated as `initialDelay * (delayMultiplier ^ attempt)`,
/// clamped to a maximum of [maxDelay].
///
/// Uses [clock] to calculate the total elapsed time for enforcing [maxRetryInterval].
@internal
...
/// Runs the given asynchronous [body] with exponential backoff retries.
///
/// Only transient gRPC errors are retried:
/// - [StatusCode.aborted]
...
/// - [StatusCode.unknown]
///
/// If [isIdempotent] is `false`, the operation is never retried and the
/// first encountered error is immediately rethrown.
///
/// The retry delay sequence is generated by [delaySequence] based on [settings].
/// If the delay sequence is exhausted and the operation still fails, the last
/// encountered exception is rethrown.
