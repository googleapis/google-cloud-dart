...
final class Subscription {
  ...
  void _initBatchers() {
    ...
    _modifyAckBatcher = Batcher<_ModifyAckDeadlineRequest>(
      settings: ackSettings.batching,
      itemSize: (req) => req.ackId.length + 4,
  ...
  Future<void> _onModifyAckBatch(List<_ModifyAckDeadlineRequest> batch) async {
    // Group by ackDeadlineSeconds, as the RPC takes one deadline for all
    // ackIds in the request.
  ...
  ///
  /// The client automatically streams acknowledgments and ack deadline
  /// modifications back to the server.
  ///
  /// If [maxConcurrentStreams] is greater than 1, multiple parallel gRPC
  /// streams are opened. This can increase throughput for high-volume
  /// subscriptions by bypassing single-stream limitations.
  ///
  /// The stream automatically reconnects on transient network errors using the
  /// configured [retry] settings (defaulting to [AckSettings.retry]).
  /// Reconnections use exponential backoff, which resets once a connection
  /// successfully yields messages.
  ///
  /// ACKs and deadline modifications sent via [acknowledge] and
  /// [modifyAckDeadline] are batched in the background and sent over the
  /// active streams. If all streams are down, they fall back to unary RPCs.
  ///
  ...
  /// * [ServiceException] if a connection fails permanently (exhausts retries).
  ...
  ///
  /// This method buffers the acknowledgment and batches it with others
  /// according to [AckSettings.batching].
  ///
  ...
  ///
  /// Buffers and batches the request with others according to
  /// [AckSettings.batching], grouping them by [ackDeadlineSeconds].
  ///
  /// If a [streamingPull] stream is active, the batched requests are sent over
  /// the stream. Otherwise, they are sent via a unary RPC with retries
