...
Future<void> main() async {
  // Initialize the token provider function with the required scopes.
  final tokenProvider = applicationDefaultCredentials(
    scopes: ['https://www.googleapis.com/auth/pubsub'],
  );

  // Pass the token provider to the PubSub client constructor.
  final pubsub = PubSub(
  ...
  try {
    // The client will use the token provider to make authenticated requests.
    final topic = pubsub.topic('my-topic');
    print('Successfully initialized client for topic: ${topic.name}');
  } on CredentialsException catch (e) {
    print('Failed to obtain credentials: ${e.message}');
  } finally {
    await pubsub.close();
  }
}
